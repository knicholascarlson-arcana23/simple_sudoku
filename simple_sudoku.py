...
for r in range(9):
    for c in range(9):
        # FIX: Use .get() to prevent Streamlit KeyErrors
...
if error_check_on:
    # FIX: Safely load the grid using .get()
    ...
    for error in unique_errors:
        st.error(error)import streamlit as st
...
for r in range(9):
    for c in range(9):
        ...
        # Only export locked-in, single digits. Pencil marks and blanks become '0'
        if len(val) == 1 and val.isdigit():
            current_puzzle_string += val
        else:
            current_puzzle_string += "0"
...
with st.expander("💾 Save / Load Puzzle Options"):
    st.write("**Export Current Board:**")
    st.code(current_puzzle_string) # Shows the string for easy copy/pasting
    ...
    if st.button("Load String"):
        ...
        if len(clean_string) == 81 and clean_string.isdigit():
            ...
            st.session_state.locked = False
            st.session_state.original_puzzle = {}
            # Fill the board with the new string
            idx = 0
            for r in range(9):
                for c in range(9):
                    char = clean_string[idx]
